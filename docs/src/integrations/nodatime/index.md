...
In OpenAPI each of the seven types documents itself with its own value-type name and an example its own
parser accepts. That mapping has one home — the
[types and examples table](../aspnetcore/openapi/#types-and-examples) — and a copy of it here would only
drift away from it.
