...
/// <summary>
///     A canceled token faults the read with <see cref="OperationCanceledException" /> — it does not become a
...
///         <c>TestContext.Current.CancellationToken</c> threaded as an argument and never canceled, and the two
...
///         widening to <c>catch (Exception)</c> turns every canceled request into a 400 and merges green.
///     </para>
...
public sealed class CancellationTests(SqliteFixture fixture) : IClassFixture<SqliteFixture> {
	...
	[Fact]
	public async Task A_canceled_token_faults_the_database_leg() {
	...
	[Fact]
	public async Task A_canceled_token_faults_the_in_memory_leg() {
	...
	[Fact]
	public async Task A_canceled_token_wins_over_an_invalid_request() {
	...
	[Fact]
	public async Task A_token_canceled_while_the_rows_are_read_stops_the_post_map() {

		using var cts = new CancellationTokenSource();

		int postMapCalls = 0;

		// The selector runs during enumeration, so the token is canceled after the rows are in memory but before
