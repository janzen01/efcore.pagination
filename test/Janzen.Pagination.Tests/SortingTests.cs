...
public sealed class SortingTests(SqliteFixture fixture) : IClassFixture<SqliteFixture> {
	...
	public async Task Direction_is_case_insensitive(string sort) {
		Assertions.HasIds(await Page(Query.Sort(sort)), 8, 7, 6, 5, 4, 3, 2, 1);
	}

	[Fact]
	public async Task A_missing_direction_is_rejected() {
		Assert.Equal("Sort value 'rank' must use the format 'field:ASC' or 'field:DESC'.", await Rejects(Query.Sort("rank")));
	}

	[Fact]
	public async Task An_unknown_direction_is_rejected() {
		Assert.Equal("Sort direction 'UP' is not supported.", await Rejects(Query.Sort("rank:UP")));
	}

	[Fact]
	public async Task An_unsortable_field_is_rejected() {
		Assert.Equal("Sort for field 'nope' is not configured.", await Rejects(Query.Sort("nope:ASC")));
	}

	[Fact]
	public async Task Sorts_are_applied_in_the_order_given() {
		// Draft(0) then Active(1) then Discontinued(2), each by descending rank.
		Assertions.HasIds(await Page(Query.Sort("status:ASC", "rank:DESC")), 5, 3, 8, 7, 4, 2, 1, 6);
	}
	...
	public async Task The_tie_breaker_orders_rows_the_primary_sort_ties() {
		...
		Assertions.HasIds(await Page(Query.Sort("status:ASC")), 3, 5, 1, 2, 4, 7, 8, 6);
	}
	...
	public async Task Defaults_apply_when_the_request_sorts_by_nothing() {
		Assertions.HasIds(await Page(new PaginateQuery { Limit = Query.All }), 1, 2, 3, 4, 5, 6, 7, 8);
	}
	...
	public async Task A_requested_sort_replaces_the_defaults_rather_than_extending_them() {
		...
		Assertions.HasIds(await Page(Query.Sort("reviewCount:DESC")), 1, 2, 3, 4, 5, 6, 7, 8);
	}
	...
	public async Task An_unsortable_field_is_rejected_even_when_the_filter_matches_nothing() {
		Assert.Equal("Sort for field 'nope' is not configured.", await Rejects(MatchingNothing("nope:ASC")));
	}

	[Fact]
	public async Task An_unsortable_field_is_rejected_even_past_the_last_page() {
		Assert.Equal("Sort for field 'nope' is not configured.", await Rejects(new PaginateQuery { Page = 999, SortBy = ["nope:ASC"] }));
	}
	...
	public async Task A_repeated_sort_field_is_rejected_before_the_query_runs() {
		...
		Assert.Equal("Sort field 'rank' is specified more than once.", await Rejects(MatchingNothing("rank:ASC", "rank:DESC")));
	}
	...
	public async Task A_config_with_only_a_tie_breaker_orders_by_it_even_when_nothing_matches() {
		...
		var page = await Page(MatchingNothing(), Unordered);
