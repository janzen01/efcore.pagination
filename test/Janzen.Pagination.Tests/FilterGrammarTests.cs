...
public sealed class FilterGrammarTests(SqliteFixture fixture) : IClassFixture<SqliteFixture> {
	...
	public async Task Unknown_operator_is_rejected() {
		Assert.Equal("Filter 'name' uses unknown operator '$foo'.", await Rejects(Query.Filter("name", "$foo:x")));
	}

	[Fact]
	public async Task Value_without_an_operator_is_rejected() {
		Assert.Equal("Filter 'name' must use the format '$operator:value'.", await Rejects(Query.Filter("name", "Widget")));
	}

	[Fact]
	public async Task Operator_without_a_value_is_rejected() {
		Assert.Equal("Filter 'name' must use the format '$operator:value'.", await Rejects(Query.Filter("name", "$eq")));
	}

	[Fact]
	public async Task Empty_criterion_is_rejected() {
		Assert.Equal("Filter 'name' must not be empty.", await Rejects(Query.Filter("name", "")));
	}

	[Fact]
	public async Task Unknown_field_is_rejected() {
		Assert.Equal("Filter for field 'nope' is not configured.", await Rejects(Query.Filter("nope", "$eq:x")));
	}

	[Fact]
	public async Task Operator_not_granted_for_the_field_is_rejected() {
		Assert.Equal("Filter 'rank' does not support operator '$ilike'.", await Rejects(Query.Filter("rank", "$ilike:x")));
	}
	...
	public async Task Null_is_also_the_one_operator_that_refuses_a_value(string criterion) {
		...
		Assert.Equal("Filter 'discontinuedAt' does not take a value for '$null'.", await Rejects(Query.Filter("discontinuedAt", criterion)));
	}
	...
	public async Task Everything_after_the_operator_colon_is_the_value() {
		...
		Assertions.HasIds(await Page(Query.Filter("name", "$eq:Doohickey: legacy")), 6);
	}

	[Fact]
	public async Task Operator_tokens_are_case_insensitive() {
		Assertions.HasIds(await Page(Query.Filter("status", "$EQ:Draft")), 3, 5);
	}
	...
	public async Task Not_negates_the_criterion() {
		Assertions.HasIds(await Page(Query.Filter("status", "$not:$eq:Active")), 3, 5, 6);
	}

	[Fact]
	public async Task Criteria_on_one_field_default_to_and() {
		Assertions.HasIds(await Page(Query.Filter("rank", "$gte:20", "$lte:40")), 2, 3, 4);
	}

	[Fact]
	public async Task Or_joins_criteria_on_one_field() {
		Assertions.HasIds(await Page(Query.Filter("status", "$eq:Draft", "$or:$eq:Discontinued")), 3, 5, 6);
	}

	[Fact]
	public async Task And_can_be_written_explicitly() {
		Assertions.HasIds(await Page(Query.Filter("rank", "$gte:20", "$and:$lte:40")), 2, 3, 4);
	}
	...
	public async Task Prefixes_may_come_in_either_order(string second) {
		Assertions.HasIds(await Page(Query.Filter("status", "$eq:Draft", second)), 3, 5, 6);
	}

	[Fact]
	public async Task Different_fields_are_always_joined_with_and() {
		Assertions.HasIds(await Page(Query.Filters(("status", "$eq:Active"), ("rank", "$gt:50"))), 7, 8);
	}
	...
	public async Task A_connector_still_reads_after_a_negation_on_a_later_criterion() {
		...
		Assertions.HasIds(await Page(Query.Filter("status", "$eq:Draft", "$not:$or:$eq:Active")), 3, 5, 6);
	}
	...
	public async Task A_list_operator_no_longer_trims_its_entries() {
		...
		var listed = await Page(Query.Filter("name", "$in: Widget"));
		var single = await Page(Query.Filter("name", "$eq: Widget"));
	...
	public async Task Two_keys_resolving_to_one_field_are_rejected() {
		...
		Assert.Equal("Filter for field 'Status' repeats 'status'; combine the criteria in one entry.", await Rejects(request));
	...
	public async Task An_unpadded_entry_beside_a_padded_one_still_matches() {
		...
		Assertions.HasIds(await Page(Query.Filter("name", "$in:Widget, Gizmo")), 1);
	}
	...
	public async Task A_padded_entry_on_a_numeric_field_still_parses() {
		...
		Assertions.HasIds(await Page(Query.Filter("rank", "$btw:20 , 40")), 2, 3, 4);
	}
	...
	public async Task Repeated_modifiers_parse_to_the_same_criterion(int prefixes) {
		...
		Assertions.HasIds(await Page(Query.Filter("status", value)), 3, 5, 6);
	}
