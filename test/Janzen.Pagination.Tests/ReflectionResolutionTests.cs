...
/// <summary>
///     A canary, not a behavior test. The engine resolves framework and provider methods by name and — in
///     three places — by parameter count, from <c>static readonly</c> field initializers. Every one of them is
...
///     is a type initializer the consumer sees a <c>TypeInitializationException</c> naming the <i>type</i>,
///     at first query, in production — while <c>dotnet build</c> and <c>dotnet pack</c> see nothing.
///     <para>
///         Touching each type's initializer here moves that failure onto the dependency-bump pull request,
...
public sealed class ReflectionResolutionTests : IDisposable {
	...
	private static void AssertEveryMethodResolved(Type type) {
		...
		// Runs the initializer first, so an ambiguous Single(...) surfaces here rather than inside GetValue.
